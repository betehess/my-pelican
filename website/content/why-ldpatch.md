...
Click on the following graph for a full size visual representation of the graph:

[![TimBL's card](/2014/09/timbl-card.png)](/2014/09/timbl-card.png)

As a side note, let me bring your attention on the URIs used here: there all resolve to actual documents on the Web, including those from [schema.org](https://schema.org/) and Facebook's [Open Graph Protocol](http://ogp.me/).
...
The group was largely in favor of (a), while (b) had basically no support. Knowing that, the group could now focus on the other proposals, like SparqlPatch.


SparqlPatch {id=sparqlpatch}
-----------

[SparqlPatch](http://www.w3.org/2001/sw/wiki/SparqlPatch) was proposed by Eric Prud'hommeaux, an editor for the SPARQL query language. SparqlPatch is a profile for SPARQL Update, as it is defined as a subset of it: a valid SparqlPatch query is always a valid SPARQL Update query, sharing the same semantics.
...
Also, it would be nice to change the evaluation semantics of the Basic Graph Pattern such that the evaluation order of the clauses is the one from the query. In the context of PATCH, it makes sense to let the client have some control over that.

<span id="confusing-semantics">SPARQL Update can also be confusing</span> in that if a graph pattern doesn't match anything, the query still succeeds with no effect on the graph. I have seen many engineers get puzzled by this (perfectly well defined) behaviour, because they were expecting a failure, and it happens everytime a predicate gets typoed. I am jumping a bit ahead but that is one reason why **LD Patch cannot be compiled down to SPARQL Update while preserving the semantics**.
...
On the other hand, lists are a common data structure in all applications. They come with native support in syntaxes like Turtle or JSON-LD. Append is a very common operation and should have to think about the [RDF list encoding](http://www.w3.org/2006/Talks/0524-Edinburgh-IH/#(54)) for such simple operations.

Limited node matching and native `rdf:list` support are two strengths of LD Patch.
...
Pierre-Antoine maintains a Python implementation, and I have a Scala implementation working with both Jena and Sesame. [Andrei Sambra](https://deiu.rww.io/profile/card#me), the third editor, is working on a Go and Javascript implementations.
...
The runtime semantics for LD Path expressions only relies on a node set. The final set must have a unique value to successfuly bind the variable, otherwise this is an error. A path expression is processed from left to right (steps), and can have nested paths for filtering nodes.

Given that semantics, you can imagine that this is easy to reason about, [easy to implement](https://github.com/w3c/banana-rdf/blob/2fb79a94c9cb52201daab4bc8608ea819706b5c1/ldpatch/src/main/scala/Semantics.scala#L158-L192), and very efficient. I would argue that you cannot remove functionalities from the path expressions without throwing away a whole class of interesting RDF graphs to patch.

Writing a parser for LD Patch proved to be of similar difficulty than for SparqlPatch, as they share most of their respective grammars with Turtle. Most of the code for the engine itself actually lies in the support for `rdf:list`, which basically encodes what users would have to do in their query if they didn't have native support. So this end up being done in one place, once and for all, and that is indeed a very good thing.
...
It took a very long time for the group to decide on LD Patch. I still regret that *any opportunity* is taken to challenge the whole technology, often without even providing which requirements they want to address.

For example, the main criticism seems to be about the syntax. Yes, it is new. In particular, it is different from the SPARQL Update syntax. But apparently, it doesn't matter to some folks if the semantics are not the same. As it doesn't matter that some requirements may not be addressed.
...
If you want to make counter proposals, please make sure that those requirements are addressed. Also, you should accept the fact that if you have a different set of requirements, then LD Patch is probably not what you want. Finally, if you think that the above requirements are **wrong in the context of LDP**, then you should make an official complain to the group explaining your reasoning.

I would like to emphasize that **relying on an existing syntax (such as SPARQL) was never a requirement for me**. While reusing bits of SPARQL Update in LD Patch whenever it makes sense is ok, it should be done sparingly. For example, [I argued on the LDP mailing list](http://lists.w3.org/Archives/Public/public-ldp-wg/2014Jul/thread.html#msg81) that shared syntax with different (runtime) semantics could break some user expectations.

Frequently Asked Questions {id="faq"}
--------------------------

<span id="dbooth-questions">Thanks to [David Booth](http://dbooth.org/)</span> for [providing me with well formulated questions](http://lists.w3.org/Archives/Public/public-ldp/2014Sep/0014.html). These are answers to specific questions and they do not replace the arguments in the other sections of this post.
...
*Yet another syntax similar to SPARQL would confuse newcomers?* Then maybe LD Patch is too close to SPARQL Update where it shouldn't be :-)

*Using a single language decreases development and maintenance costs?* I would like to see actual evidence. Some people actually have [a more nuanced opinion](http://martinfowler.com/bliki/OneLanguage.html) on that subject, and I tend to agree as I find myself using the language/framework that I find the most fit to a given job.
...
*Implementers would have the option of supporting additional SPARQL 1.1 Update operations?* There is definitely a use-case for querying data in LDP Container using SPARQL or a [more ad-hoc approach](http://blog.pellucid.com/post/95282190715/exposing-resources-in-datomic-using-linked-data). And it is true that bulk updating could be addressed with SPARQL Update. But those use-cases are different from PATCH.
...
In the meantime, the editors are working on completing the semantics section of the document. A proposed approach was to provide a translation from LD Patch to SPARQL Update. While this is definitely useful for people with a SPARQL background, [this cannot be used as a semantics](#confusing-semantics). We are trying to find a good trade-off between usual tooling in formal semantics, and something that could be read by people without such a formal background.

And finally, after the specification gets completed, we will focus on providing a test suite. We plan to make it part of the [LDP one](https://github.com/w3c/ldp-testsuite).